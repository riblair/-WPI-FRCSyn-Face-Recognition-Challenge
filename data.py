...
class DataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: str) -> None:
        ...
        genders = ["Female", "Male"]
        dcface_root = self.datasets_root / "Synth" / "DCFace"
        dcface_dirs = []
        for i in range(0,10000):
            dcface_dirs.append( dcface_root / str(i))
        gandiffface_root = self.datasets_root / "Synth" / "GANDiffFace"
        gandiffface_dirs = [
        ...
        if self.augment:
            train_transform = A.Compose([
                ...
                A.RandomResizedCrop(112, 112, scale=(0.2, 1.0), ratio=(0.75, 1.333), p=0.2), # increase prob (slightly more)
                A.ColorJitter(brightness=0.5, contrast=0.5, saturation=0.5, hue=0, p=0.2), # increase prob (slightly)
                A.Compose([
